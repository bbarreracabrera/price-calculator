      ...
      <div style={{ maxWidth: 720, margin: '0 auto', background: '#020617', borderRadius: 12, padding: 24 }}>
        ...
        {!profile?.is_pro && (
          <div style={{ background: '#1e293b', padding: 16, borderRadius: 8, marginBottom: 24 }}>
            <p style={{ color: '#facc15' }}>Estás en el plan Free. Máximo 3 cálculos.</p>
            <button
              onClick={async () => {
                if (!profile) return
                try {
        ...
        {profile?.is_pro && (
          <>
            <h2 style={{ fontSize: 20, marginBottom: 12 }}>Historial</h2>
            {calculations.length === 0 ? (
              <p style={{ color: '#94a3b8' }}>No hay cálculos aún</p>
            ) : (
              <ul style={{ listStyle: 'none', padding: 0 }}>
                {calculations.map(c => (
                  <li key={c.id} style={{ padding: 12, background: '#1e293b', borderRadius: 8, marginBottom: 8 }}>
